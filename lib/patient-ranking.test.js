...
describe("sortPatients", () => {
  ...
  it("should sort patients based ont their score in a descending order if listSize === length of patients", () => {
    const sortedPatients = sortPatients(patientsWithScore, 3, 0.3);
  ...
  it("should sort patients based on their score and include a few patients with little behavioral data (if any) in the final list if listSize < length of patients", () => {
    const sortedPatients = sortPatients(patientsWithScore, 2, 0.3);
...
describe("generatePatientList", () => {
  const patientList = generatePatientList(
    PATIENTS_TEST_DATA,
    FACILITY_LOCATION,
    3
  );

  it("should return a list of patients with score", () => {
    expect(patientList).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          distanceToPractice: expect.anything(),
          age: expect.anything(),
          acceptedOffers: expect.anything(),
          canceledOffers: expect.anything(),
          averageReplyTime: expect.anything(),
          score: expect.anything(),
          littleBehaviorData: expect.anything(),
        }),
      ])
    );
  });
