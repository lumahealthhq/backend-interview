...
exports.generatePatientList = (patients, facilityLocation, listSize = 10) => {
  ...
  // given the ranges, calculate the score of each patient and flag a patient if it has little behavioral data
...
exports.findPatientDataRanges = (
  ...
  patients.forEach((patient) => {
    for (let category in SCORE_COMPOSITION) {
      ...
      // calculating category bottom threshold, which defaults to the bottom ten percent (bottomThreshold parameter)
...
exports.sortPatients = (
  patients,
  finalListLength,
  littleBehaviorDataPercentage = 0.1
) => {
  ...
  // checking if we can fill the 10% (littleBehaviorDataPercentage param) of the list with littleBehaviorDataPatients
  if (
