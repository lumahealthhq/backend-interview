...
def evaluate_age_data(data):
    ...
    sorted_age_data.sort(
        key=lambda tup: tup[0]
    )  # not enough significance in the all years data to make it a useable metric for acceptance likelyhood (high varience year to year)
...
# quick check to see check how close the accepted and cancled offers are to expected values
# also explored using a ratio as the feature for accepted and canceled offers
    ...
    for p in data:
        if p["canceledOffers"] != 0:
            avg_ratio += (
                p["acceptedOffers"]
                / p["canceledOffers"]
                * (p["acceptedOffers"] + p["canceledOffers"])
            )
