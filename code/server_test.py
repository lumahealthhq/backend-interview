...
class TestServerResponses(unittest.TestCase):
	def setUp(self):
		...
		api.add_resource(server.Top_Patients, '/patients', resource_class_kwargs={"data": scored_data}) # Route_1
	...
	def test_post(self):
		...
		item = {"longitude": -13}
		response = self.app.post("http://127.0.0.1:5002/patients",
			data=item)
		
		self.assertEqual(response.status_code, 400)
		data = json.loads(response.get_data())
		self.assertEqual(data, {"error": "Invalid Argument: latitude missing from input"})
		...
		self.assertEqual(response.status_code, 400)
		data = json.loads(response.get_data())
		self.assertEqual(data, {"error": "Invalid Argument: longitude missing from input"})
		...
		item = {"latitude": "asdad", "longitude":"asdfasd"} #lets pass in giberish and force a server error
