...
def load_data(path="../sample-data/patients.json"):
    with open(path) as f:
        return json.load(f)


# get means and standard deviations, used for scoring below
...
# calculate scores intrinsic to the patient, we can reuse these scores for multiple api calls from different locations
def calculate_intrinsic_score(
    ...
    for x in range(len(data)):
        if data[x]["age"] <= 55:
            ...
            data[x]["score"] = (
                age_weight * 2 / 2.94
            )  # based this of discovery from data exploration that younger accept more often
        ...
        data[x]["score"] += accepted_weight * st.norm.cdf(
            (data[x]["acceptedOffers"] - accepted_mean) / accepted_std
        )
        data[x]["score"] += canceled_weight * (
            1 - st.norm.cdf((data[x]["canceledOffers"] - canceled_mean) / canceled_std)
        )
        data[x]["score"] += response_time_weight * st.norm.cdf(
            (data[x]["averageReplyTime"] - response_time_mean) / response_time_std
        )
    return data


# convenience function for server
def prepare_data(path="../sample-data/patients.json"):
    patient_data = load_data(path)
    accepted_mean, accepted_std, canceled_mean, canceled_std, response_time_mean, response_time_std = get_data_stats(
        patient_data
    )
    scored_data = calculate_intrinsic_score(
        patient_data,
        accepted_mean,
        accepted_std,
        canceled_mean,
...
# takes the pre-calculated intrinsic score and finalizes the score based on location
# also handles randomly adding low appointment data customers
def final_score(hospital_location, scored_data, n=10):
    ...
    for d in data:
        ...
        d["score"] += max(
            distance_weight * (3000 - distance) / 3000, 0
        )  # the random distances lead to very large distnaces, capping the scoring function at 3000miles otherwise awarding no points
        ...
        if d["acceptedOffers"] + d["canceledOffers"] < 50:
            ...
            bonus_cancelation_check = random.randint(
                -d["canceledOffers"], d["acceptedOffers"]
            )
